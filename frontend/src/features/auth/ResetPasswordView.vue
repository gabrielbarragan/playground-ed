<template>
  <div class="auth-page">
    <div class="auth-card">
      ...
      <!-- Token inválido (no llegó en la URL) -->
      <div v-if="!token" class="error-box">
        <p>Link inválido. Por favor solicitá uno nuevo.</p>
        <RouterLink to="/forgot-password" class="auth-link">Solicitar nuevo link</RouterLink>
      </div>
      ...
      <div v-else class="success-box">
        ...
        <p class="success-text">¡Contraseña actualizada correctamente!</p>
        <RouterLink to="/login" class="btn-submit" style="margin-top:1rem;text-decoration:none">
          Ir al login
        </RouterLink>
      </div>
...
async function handleSubmit() {
  ...
  try {
    ...
    error.value = e?.response?.data?.detail ?? 'Token inválido o expirado. Solicitá un nuevo link.'
  } finally {
    loading.value = false
  }
}
